...
public class LeetCodeAllQuestionLatest {
	private static String dirPath = "";
	private static String excelPath = "C:\\Leetcode\\leetcode-excel";
	...
	public static Map<String, Map<Integer, Integer>> startAll(Map<String, String> companyMap) throws Exception {
		LeetCodeAllQuestionLatest lc = new LeetCodeAllQuestionLatest();
		Map<String, List<LeetCodeRow>> allMap = new HashMap<>();
		Map<String, Map<Integer, List<Integer>>> all = new HashMap<>();
		Map<String, Map<Integer, Integer>> companyFrequencyMap = new HashMap<>();
		for (Entry<String, String> entry : companyMap.entrySet()) {
			lc.processAllFiles(allMap, all, entry.getKey(), entry.getValue(), companyFrequencyMap);
		}
	...
	private void processAllFiles(Map<String, List<LeetCodeRow>> allMap, Map<String, Map<Integer, List<Integer>>> all,
			File file, Map<String, Map<Integer, Integer>> companyFrequencyMap) throws Exception {
		String jsonString = new String(Files.readAllBytes(file.toPath()));
		String fileName = FilenameUtils.getBaseName(file.getName());
		processAllFiles(allMap, all, fileName, jsonString, companyFrequencyMap);
	}

	private void processAllFiles(Map<String, List<LeetCodeRow>> allMap, Map<String, Map<Integer, List<Integer>>> all,
			String fileName, String jsonString, Map<String, Map<Integer, Integer>> companyFrequencyMap)
			throws Exception {
		List<LeetCodeRow> list = new ArrayList<>();
		Map<Integer, Integer> frequencyMap = new HashMap<>();
		companyFrequencyMap.put(fileName, frequencyMap);
		list.add(new LeetCodeRow("FrontendId", "QuestionId", "Question", "Difficulty", "Frequency 6 month",
				"Frequency 1 Year", "Frequency 2 Year", "Frequency All", "PaidOnly"));
		...
		processAllData(dlb, list, frequencyMap);
		allMap.put(fileName, list);
	}

	private void processAllData(AllQuestionMain dlb, List<LeetCodeRow> list, Map<Integer, Integer> frequencyMap) {
		...
		for (Question ques : questions) {
			...
			frequencyMap.put(ques.getQuestionId(), freq.get(0));
			LeetCodeRow leetCodeRow = new LeetCodeRow(ques.getQuestionFrontendId(), ques.getQuestionId(),
					ques.getTitle(), ques.getDifficulty(), freq.get(0), freq.get(1), freq.get(2), freq.get(3), null);
	...
	private void updateToExcel(Map<String, List<LeetCodeRow>> data) throws Exception {
		...
		try {
			...
			File file = new File(excelPath + "\\LeetCode" + ".xlsx");
			if(!file.exists()) {
	...
	public static void mainDeprecated(String[] args) throws Exception {
		...
		for (File file : dir.listFiles()) {
			if (file.isFile())
				lc.processFilesDeprecated(all, map, file);
		}
		lc.writeToExcel(map);
		// startAll();
	...
	public static void startAll(String fileName, String jsonString) throws Exception {
		...
		Map<String, Map<Integer, List<Integer>>> all = new HashMap<>();
		Map<String, Map<Integer, Integer>> companyFrequencyMap = new HashMap<>();
		lc.processAllFiles(allMap, all, fileName, jsonString, companyFrequencyMap);
		lc.updateToExcel(allMap);
	}

	@Deprecated
	private void processFilesDeprecated(Map<String, Map<Integer, List<Integer>>> all,
			Map<String, List<LeetCodeRow>> map, File file) throws Exception {
		String jsonString = new String(Files.readAllBytes(file.toPath()));
		AllQuestions dlb = JsonService.getObjectFromJson(jsonString, AllQuestions.class);
		String fileName = FilenameUtils.getBaseName(file.getName());
		List<LeetCodeRow> data = loadDataDeprecated(fileName, all, dlb);
		map.put(fileName, data);

	}

	@Deprecated
	private void loadAllFilesDeprecated(Map<String, List<LeetCodeRow>> allMap,
			Map<String, Map<Integer, List<Integer>>> all) throws Exception {
		String dirPathAll = dirPath + "\\All";
		File dir = new File(dirPathAll);
		Map<String, Map<Integer, Integer>> companyFrequencyMap = new HashMap<>();
		for (File file : dir.listFiles()) {
			if (file.isFile())
				processAllFiles(allMap, all, file, companyFrequencyMap);
		}
	}
	...
	public static void startAll() throws Exception {
		...
		Map<String, List<LeetCodeRow>> allMap = new HashMap<>();
		Map<String, Map<Integer, List<Integer>>> all = new HashMap<>();
		lc.loadAllFilesDeprecated(allMap, all);
		lc.updateToExcel(allMap);
	}

	@Deprecated
	private List<LeetCodeRow> loadDataDeprecated(String fileName, Map<String, Map<Integer, List<Integer>>> all,
			AllQuestions dlb) {
		List<LeetCodeRow> list = new ArrayList<>();
		...
		for (StatStatusPair statStatusPair : dlb.getStatStatusPairs()) {
			...
			leetCodeRow.setAmazonFrequency(Optional.ofNullable(all.get("Amazon-All"))
					.map(x -> x.get(statStatusPair.getStat().getQuestionId())).map(y -> y.get(0)).orElse(0));
			leetCodeRow.setAppleFrequency(Optional.ofNullable(all.get("Apple-All"))
					.map(x -> x.get(statStatusPair.getStat().getQuestionId())).map(y -> y.get(0)).orElse(0));
			leetCodeRow.setFacebookFrequency(Optional.ofNullable(all.get("Facebook-All"))
					.map(x -> x.get(statStatusPair.getStat().getQuestionId())).map(y -> y.get(0)).orElse(0));
			leetCodeRow.setGoogleFrequency(Optional.ofNullable(all.get("Google-All"))
					.map(x -> x.get(statStatusPair.getStat().getQuestionId())).map(y -> y.get(0)).orElse(0));
			leetCodeRow.setLinkedinFrequency(Optional.ofNullable(all.get("LinkedIn-All"))
					.map(x -> x.get(statStatusPair.getStat().getQuestionId())).map(y -> y.get(0)).orElse(0));
			leetCodeRow.setMicrosoftFrequency(Optional.ofNullable(all.get("Microsoft-All"))
					.map(x -> x.get(statStatusPair.getStat().getQuestionId())).map(y -> y.get(0)).orElse(0));
